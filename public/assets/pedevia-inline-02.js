...
function openProduct(id){let p=cfg.products.find(x=>x.id===id),h=`<div class="row"><div><h2 style="margin:0">${esc(p.name)}</h2><span class="price">${brl(p.price)}</span></div><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div><p>${esc(p.desc)}</p>`;
 ...
 h+=`<label>Observação</label><textarea id="itemObs" class="field" placeholder="Ex.: sem granola"></textarea><div class="row"><div class="qty"><button data-pedevia-handler="h001">−</button><b id="qty">1</b><button data-pedevia-handler="h002">+</button></div><button class="btn" data-pedevia-handler="h003" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Adicionar</button></div>`;showModal(h)}
...
function showCart(){let sub=sum(),fee=+cfg.store.deliveryFee;let h=`<div class="row"><h2>Carrinho</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>`;if(!cart.length){showModal(h+"<p>Seu carrinho está vazio.</p>");return}
 h+=cart.map((i,n)=>{let p=cfg.products.find(x=>x.id===i.pid);return`<div class="summary"><div class="row"><b>${i.qty}x ${esc(p.name)}</b><b>${brl(i.unit*i.qty)}</b></div>${i.groups.map(g=>`<small><b>${g.name}:</b> ${g.items.map(x=>esc(x.name)).join(", ")}</small>`).join("")}${i.obs?`<small>Obs.: ${esc(i.obs)}</small>`:""}<button class="ghost" style="margin-top:8px" data-pedevia-handler="h004" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">Remover</button></div>`}).join("");
...
function renderAdmin(){
 $("#adminBottomNav").innerHTML=adminTabs.map(([id,ic,n])=>`<button class="${adminTab===id?"on":""}" data-pedevia-handler="h005" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}"><span class="navIcon">${ic}</span><span>${n}</span></button>`).join("");
 ...
 }catch(e){console.error(e);$("#adminContent").innerHTML=`<div class="panel"><h3>Não foi possível abrir esta área</h3><p class="hint">Os dados foram preservados. Toque no botão abaixo para reparar o cardápio e tentar novamente.</p><button class="btn full" data-pedevia-handler="h006">Reparar cardápio</button></div>`}
}
function adminHome(){
...
function adminMenu(){
 $("#adminContent").innerHTML=`<div class="adminSubnav"><button class="${adminSubTab==="products"?"on":""}" data-pedevia-handler="h007">Produtos</button><button class="${adminSubTab==="stock"?"on":""}" data-pedevia-handler="h008">Complementos e estoque</button></div><div id="adminSection"></div>`;
 let target=$("#adminContent"), sub=$("#adminSection");
 if(adminSubTab==="products"){adminProducts(); let rendered=$("#adminContent").innerHTML; $("#adminContent").innerHTML=`<div class="adminSubnav"><button class="on" data-pedevia-handler="h009">Produtos</button><button data-pedevia-handler="h010">Complementos e estoque</button></div>`+rendered;}
 else {adminStock(); let rendered=$("#adminContent").innerHTML; $("#adminContent").innerHTML=`<div class="adminSubnav"><button data-pedevia-handler="h011">Produtos</button><button class="on" data-pedevia-handler="h012">Complementos e estoque</button></div>`+rendered;}
}
function adminOrders(){
 ...
 <div class="orderBox"><h3>Pedidos enviados pelos clientes</h3><p>Chegam no WhatsApp ${formatWa(cfg.store.whatsapp)}</p><button class="ghost" data-pedevia-handler="h013">Configurações de pedidos</button></div>
...
function adminOrderSettings(){let s=cfg.store,o=s.orderConfig;
 $("#adminContent").innerHTML=`<button class="backBtn" data-pedevia-handler="h014">‹ Voltar para Pedidos</button><div class="panel"><div class="settingsTitle">Configurações de pedidos</div><div class="settingsList">
...
function settingCard(icon,title,sub,action){return `<button class="settingCard" data-pedevia-named-action="${encodeURIComponent(action)}"><span class="si">${icon}</span><span><b>${title}</b><small>${sub}</small></span><span class="chev">›</span></button>`}
...
function adminProducts(){$("#adminContent").innerHTML=`<div class="panel"><div class="row"><h3>Produtos</h3><button class="btn" data-pedevia-event="click" data-pedevia-call="newProduct">+ Novo</button></div>${cfg.products.map(p=>`<div class="adminItem"><div><b>${esc(p.name)}</b> · ${brl(p.price)}<br><span class="badge ${p.status}">${statusLabel(p.status)}</span> <span class="hint">${cfg.categories.find(c=>c.id===p.category)?.name||""}</span></div><button class="ghost" data-pedevia-handler="h020" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Editar</button></div>`).join("")}</div>`}
function editProduct(id){let p=cfg.products.find(x=>x.id===id);showModal(`<div class="row"><h2>Editar produto</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div><label>Nome</label><input id="epn" class="field" value="${esc(p.name)}"><label>Descrição</label><textarea id="epd" class="field">${esc(p.desc)}</textarea><label>Preço</label><input id="epp" type="number" step=".01" class="field" value="${p.price}"><label>Categoria</label><select id="epc" class="field">${cfg.categories.map(c=>`<option value="${c.id}" ${c.id===p.category?"selected":""}>${c.name}</option>`).join("")}</select><label>URL da imagem</label><input id="epi" class="field" value="${esc(p.image)}"><label>Status</label><select id="eps" class="field"><option value="available">Disponível</option><option value="unavailable">Indisponível</option><option value="hidden">Oculto</option></select><div class="group"><div class="groupTitle">Opções deste produto</div>${cfg.groups.map(g=>`<label class="option"><span>${g.name}</span><input class="pg" type="checkbox" value="${g.id}" ${p.groups.includes(g.id)?"checked":""}></label>`).join("")}</div><button class="btn full" data-pedevia-handler="h021" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Salvar</button>`);$("#eps").value=p.status}
...
function adminStock(){$("#adminContent").innerHTML=cfg.groups.map(g=>`<div class="panel"><div class="row"><div><h3>${g.name}</h3><span class="hint">mín. ${g.min} · máx. ${g.max}</span></div><button class="ghost" data-pedevia-handler="h022" data-pedevia-args="${encodeURIComponent(JSON.stringify([g.id]))}">Regras</button></div>${g.options.map(o=>`<div class="adminItem"><div><b>${esc(o.name)}</b>${o.price?" · "+brl(o.price):""}<br><span class="badge ${o.status}">${statusLabel(o.status)}</span></div><div class="miniBtns"><button data-pedevia-handler="h023" data-pedevia-args="${encodeURIComponent(JSON.stringify([g.id,o.id]))}">✓</button><button data-pedevia-handler="h024" data-pedevia-args="${encodeURIComponent(JSON.stringify([g.id,o.id]))}">×</button><button data-pedevia-handler="h025" data-pedevia-args="${encodeURIComponent(JSON.stringify([g.id,o.id]))}">👁</button></div></div>`).join("")}<button class="ghost" data-pedevia-handler="h026" data-pedevia-args="${encodeURIComponent(JSON.stringify([g.id]))}">+ Adicionar item</button></div>`).join("")}
function setOpt(gid,oid,s){cfg.groups.find(g=>g.id===gid).options.find(o=>o.id===oid).status=s;save();adminStock();renderShop()}
function editRules(id){let g=cfg.groups.find(x=>x.id===id);showModal(`<div class="row"><h2>${g.name}</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div><label>Nome</label><input id="rname" class="field" value="${esc(g.name)}"><div class="two"><div><label>Mínimo</label><input id="rmin" type="number" class="field" value="${g.min}"></div><div><label>Máximo</label><input id="rmax" type="number" class="field" value="${g.max}"></div></div><button class="btn full" data-pedevia-handler="h027" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}">Salvar</button>`)}
...
function renderNeighborhoodRows(){let q=($('#neighSearch')?.value||'').toLowerCase(),arr=cfg.store.orderConfig.neighborhoods;$('#neighRows').innerHTML=arr.map((n,i)=>({n,i})).filter(x=>x.n.name.toLowerCase().includes(q)).map(({n,i})=>`<div class="neigh"><div><b>${esc(n.name)}</b><div class="hint">${n.enabled?'Atendido':'Desativado'}</div></div><input class="field neighFee" data-i="${i}" type="number" step=".01" value="${n.fee}"><button class="xbtn" data-pedevia-handler="h028" data-pedevia-args="${encodeURIComponent(JSON.stringify([i]))}">×</button></div>`).join('')||'<div class="notice">Nenhum bairro encontrado.</div>'}
...
function moreCard(ic,title,sub,fn){return `<button class="moreCard" data-pedevia-named-action="${encodeURIComponent(fn)}"><span class="moreIc">${ic}</span><span><b>${title}</b>${sub?`<small>${sub}</small>`:''}</span><span class="chev">›</span></button>`}
...
function adminProducts(){
  $('#adminContent').innerHTML=`<div class="panel"><div class="row"><div><h3 style="margin:0">Produtos</h3><div class="hint">Crie e edite os itens do cardápio</div></div><button class="btn" data-pedevia-event="click" data-pedevia-call="newProduct">+ Novo produto</button></div><div class="categoryPills"><button class="tab on">Todos</button>${cfg.categories.map(c=>`<button class="tab">${esc(c.name)}</button>`).join('')}</div>${cfg.products.map(p=>`<div class="adminItem"><div><b>${esc(p.name)}</b> · ${brl(p.price)}<br><span class="badge ${p.status}">${statusLabel(p.status)}</span> <span class="hint">${esc(cfg.categories.find(c=>c.id===p.category)?.name||'Produtos')} · ${p.groups.length} grupo(s)</span></div><button class="ghost" data-pedevia-handler="h037" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Editar</button></div>`).join('')}</div>`;
}
...
function editProduct(id){let p=cfg.products.find(x=>x.id===id); if(!p)return;
 ...
 <div class="prodHero" id="prodPrev">${productImagePreview(p)}</div><div class="prodImageActions"><label class="ghost" style="display:inline-block">📷 Escolher imagem<input id="prodFile" type="file" accept="image/*" hidden data-pedevia-handler="h039" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}"></label><button class="ghost" data-pedevia-handler="h040">🔗 Usar URL</button></div>
 ...
 <button class="ghost full" data-pedevia-handler="h041" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">＋ Adicionar grupo de complementos</button>
 <div class="stickySave"><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">Cancelar</button><button class="btn" data-pedevia-handler="h042" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Concluir</button></div>`); $('#eps').value=p.status;
}
...
function openAddGroupMenu(pid){let p=cfg.products.find(x=>x.id===pid);showModal(`<div class="row"><h2>Adicionar grupo de complementos</h2><button class="ghost" data-pedevia-handler="h043" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">✕</button></div><div class="productAddMenu"><button class="ghost" data-pedevia-handler="h044" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">＋ Criar um novo grupo de complementos</button><button class="ghost" data-pedevia-handler="h045" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">↻ Selecionar grupos de complementos reutilizáveis</button></div><div class="hint" style="margin-top:12px">Grupos reutilizáveis permitem manter as mesmas opções em vários copos. Alterando o grupo global, todos os produtos vinculados podem ser atualizados juntos.</div>`)}
function chooseReusableGroups(pid){let p=cfg.products.find(x=>x.id===pid);showModal(`<div class="row"><h2>Grupos reutilizáveis</h2><button class="ghost" data-pedevia-handler="h046" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">✕</button></div><div class="groupChooser">${cfg.groups.map(g=>`<label class="option"><span><b>${esc(g.name)}</b><small class="hint">${esc(groupNames(g))}</small></span><input class="reuseG" type="checkbox" value="${g.id}" ${p.groups.includes(g.id)?'checked':''}></label>`).join('')}</div><button class="btn full" data-pedevia-handler="h047" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">Aplicar ao produto</button>`)}
...
function editGroupForProduct(pid,gid){let g=cfg.groups.find(x=>x.id===gid);if(!g)return; showModal(`<div class="row"><h2>Editar grupo de complementos</h2><button class="ghost" data-pedevia-handler="h048" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">✕</button></div><label>Título do grupo *</label><input id="ggName" class="field" value="${esc(g.name)}"><div class="ruleGrid"><button class="ruleChoice ${g.selectionMode==='single'?'on':''}" data-pedevia-handler="h049">◉<br>Uma única opção</button><button class="ruleChoice ${g.selectionMode==='multiple'?'on':''}" data-pedevia-handler="h050">☑<br>Uma ou mais opções</button><button class="ruleChoice ${g.selectionMode==='quantity'?'on':''}" data-pedevia-handler="h051">− ＋<br>Opção de quantidade</button></div><input type="hidden" id="ggMode" value="${g.selectionMode||'multiple'}">
 ...
 <div class="stickySave"><button class="ghost" data-pedevia-handler="h052" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid]))}">Cancelar</button><button class="btn" data-pedevia-handler="h053" data-pedevia-args="${encodeURIComponent(JSON.stringify([pid,gid]))}">Concluir edição</button></div>`)}
function setGroupMode(m){$('#ggMode').value=m;$$('.ruleChoice').forEach((b,i)=>b.classList.toggle('on',['single','multiple','quantity'][i]===m))}
function groupOptionEditors(g){return (g.options||[]).map((o,i)=>`<div class="optionEdit"><div class="line"><input class="field goName" value="${esc(o.name)}" placeholder="Nome"><input class="field goPrice" type="number" step=".01" value="${o.price||0}" placeholder="R$"><button class="ghost" data-pedevia-handler="h054">×</button></div><input class="field goDesc" value="${esc(o.desc||'')}" placeholder="Descrição (opcional)"><select class="field goStatus"><option value="available" ${o.status==='available'?'selected':''}>Disponível</option><option value="unavailable" ${o.status==='unavailable'?'selected':''}>Indisponível</option><option value="hidden" ${o.status==='hidden'?'selected':''}>Oculto</option></select></div>`).join('')}
function addGroupOptionEditor(){$('#ggOptions').insertAdjacentHTML('beforeend',`<div class="optionEdit"><div class="line"><input class="field goName" placeholder="Nome"><input class="field goPrice" type="number" step=".01" value="0"><button class="ghost" data-pedevia-handler="h055">×</button></div><input class="field goDesc" placeholder="Descrição (opcional)"><select class="field goStatus"><option value="available">Disponível</option><option value="unavailable">Indisponível</option><option value="hidden">Oculto</option></select></div>`)}
...
function adminMenu(){
 $('#adminContent').innerHTML=`<div class="panel"><div class="menuBuilderHead"><div><h3 style="margin:0">Cardápio</h3><div class="hint">Organize produtos em seções como Açaí, Sorvetes, Biscoitos e outras.</div></div><button class="btn" data-pedevia-event="click" data-pedevia-call="newSection">＋ Adicionar seção</button></div><div class="adminSubnav"><button class="on">Seções e produtos</button><button data-pedevia-handler="h058">Complementos e estoque</button></div>${cfg.sections.map((s,i)=>`${i===0?'<div class="sectionInsert"><button data-pedevia-handler="h059">＋ Adicionar seção</button></div>':''}<div class="sectionAdmin"><div class="sectionAdminTop"><div><h3>${esc(s.title)} ${s.active?'':'<span class="badge hidden">Oculta</span>'}</h3><div class="sectionDesc">${esc(s.description||'Sem descrição')} · ${productsForSection(s.id).length} produto(s)</div></div><div class="miniBtns"><button data-pedevia-handler="h060" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}" ${i===0?'disabled':''}>↑</button><button data-pedevia-handler="h061" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}" ${i===cfg.sections.length-1?'disabled':''}>↓</button><button data-pedevia-handler="h062" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">✎</button></div></div><div class="sectionAdminProducts">${productsForSection(s.id).map(p=>`<div class="adminProdCard"><b>${esc(p.name)}</b><span class="price">${brl(p.price)}</span><div class="hint">${statusLabel(p.status)} · ${(p.groups||[]).length} grupos</div><button class="ghost full" data-pedevia-handler="h063" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Editar produto</button></div>`).join('')||'<div class="emptySection">Nenhum produto nesta seção.</div>'}</div><button class="ghost full" style="margin-top:10px" data-pedevia-handler="h064" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">＋ Adicionar produto em ${esc(s.title)}</button><button class="dangerBtn" style="margin-top:8px" data-pedevia-handler="h065" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">Remover seção</button></div><div class="sectionInsert"><button data-pedevia-handler="h066" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">＋ Adicionar seção</button></div>`).join('')}</div>`;
}
function adminProducts(){adminMenu()}

function editProduct(id){let p=cfg.products.find(x=>x.id===id); if(!p)return;
 showModal(`<div class="row"><h2>${p.name==='Novo produto'?'Adicionar produto':'Editar produto'}</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div><div class="prodHero" id="prodPrev">${productImagePreview(p)}</div><div class="prodImageActions"><label class="ghost" style="display:inline-block">📷 Escolher imagem<input id="prodFile" type="file" accept="image/*" hidden data-pedevia-handler="h067" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}"></label><button class="ghost" data-pedevia-handler="h068">🔗 Usar URL</button></div><label>Nome do produto *</label><input id="epn" class="field" value="${esc(p.name)}"><label>Descrição</label><textarea id="epd" class="field">${esc(p.desc)}</textarea><label>Descrição detalhada</label><textarea id="epdetail" class="field" placeholder="Opcional">${esc(p.detailedDesc||'')}</textarea><div class="two"><div><label>Preço (R$)</label><input id="epp" type="number" step=".01" class="field" value="${p.price}"></div><div><label>Seção do cardápio</label><select id="epc" class="field">${cfg.sections.map(s=>`<option value="${s.id}" ${s.id===p.category?'selected':''}>${esc(s.title)}</option>`).join('')}</select></div></div><label>URL da imagem</label><input id="epi" class="field" value="${esc(p.image||'')}" placeholder="https://..."><label>Status</label><select id="eps" class="field"><option value="available">Disponível</option><option value="unavailable">Indisponível</option><option value="hidden">Oculto</option></select><div class="sectionTitle">Complementos e opcionais</div><div class="subtle">Os grupos abaixo aparecem ao cliente sempre que ele abrir este produto.</div><div id="productGroupList">${productGroupsHTML(p)}</div><button class="ghost full" data-pedevia-handler="h069" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">＋ Adicionar grupo de complementos</button><div class="stickySave"><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">Cancelar</button><button class="btn" data-pedevia-handler="h070" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Concluir</button></div>`); $('#eps').value=p.status;
}
...
function openProduct(id){
  ...
  (p.groups||[]).forEach(gid=>{
    ...
    if(r.mode==='quantity') h+=os.map(o=>quantityOptionRow(g,o)).join('');
      ...
      h+=os.map(o=>`<label class="option optionCard"><span><b>${esc(o.name)}</b>${o.desc?`<small class="hint">${esc(o.desc)}</small>`:''}${o.price?`<span class="optionPrice">+ ${brl(o.price)}</span>`:'<span class="optionPrice">Grátis</span>'}</span><input type="${type}" name="g_${g.id}" value="${o.id}" data-pedevia-event="change" data-pedevia-call="handleChoiceChange" data-pedevia-arg="this"></label>`).join('');
    }
    h+='</div>';
  });
  h+=`<label>Observação</label><textarea id="itemObs" class="field" placeholder="Ex.: sem granola"></textarea><div class="productFooter"><div class="productQty"><button data-pedevia-handler="h074">−</button><b id="qty">1</b><button data-pedevia-handler="h075">+</button></div><button class="btn addPriceBtn" data-base="${+p.price||0}" data-pedevia-handler="h076" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Adicionar • <span class="liveTotal">${brl(p.price)}</span></button></div>`;
...
function showCart(){let sub=sum(),fee=+cfg.store.deliveryFee;let h=`<div class="row"><h2>Carrinho</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>`;if(!cart.length){showModal(h+'<p>Seu carrinho está vazio.</p>');return}
 h+=cart.map((i,n)=>{let p=cfg.products.find(x=>x.id===i.pid);return`<div class="summary"><div class="row"><b>${i.qty}x ${esc(p?.name||'Produto')}</b><b>${brl(i.unit*i.qty)}</b></div>${i.groups.map(g=>`<small><b>${esc(g.name)}:</b> ${g.items.map(itemLabel).join(', ')}</small>`).join('')}${i.obs?`<small>Obs.: ${esc(i.obs)}</small>`:''}<button class="ghost" style="margin-top:8px" data-pedevia-handler="h077" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">Remover</button></div>`}).join('');
 h+=`<h3>Finalizar pedido</h3><label>Nome</label><input id="cust" class="field"><label>Tipo</label><select id="orderMode" class="field" data-pedevia-event="change" data-pedevia-call="checkoutTotals">${cfg.store.modes.delivery?'<option value="delivery">Entrega</option>':''}${cfg.store.modes.pickup?'<option value="pickup">Retirada</option>':''}${cfg.store.modes.dinein?'<option value="dinein">Consumo no local</option>':''}</select><div id="addressBox"><label>Endereço, número, bairro e referência</label><input id="addr" class="field"></div><label>Pagamento</label><select id="pay" class="field">${cfg.store.payments.pix?'<option>Pix</option>':''}${cfg.store.payments.cash?'<option>Dinheiro</option>':''}${cfg.store.payments.debit?'<option>Débito</option>':''}${cfg.store.payments.credit?'<option>Crédito</option>':''}</select><label>Observação geral</label><textarea id="orderObs" class="field"></textarea><div class="summary"><div class="row"><span>Subtotal</span><b>${brl(sub)}</b></div><div class="row" id="feeLine"><span>Taxa de entrega</span><b>${brl(fee)}</b></div><div class="row"><h3>Total</h3><h3 id="grand">${brl(sub+fee)}</h3></div></div>${sub<+cfg.store.minimumOrder?`<div class="notice bad">Pedido mínimo: ${brl(cfg.store.minimumOrder)}</div>`:'<button class="btn green full" data-pedevia-event="click" data-pedevia-call="sendOrder">Enviar pedido pelo WhatsApp</button>'}`;showModal(h);checkoutTotals();}
...
function showCart(){
 ...
 h+=`<div>`+cart.map((i,n)=>{let p=cfg.products.find(x=>x.id===i.pid);return `<div class="cartItem"><div class="cartItemHead"><div><b>${esc(p?.name||'Produto')}</b><div class="cartItemMeta">${i.groups.map(g=>`<div>• ${esc(g.name)}: ${g.items.map(itemLabel).join(', ')}</div>`).join('')}${i.obs?`<div>• Obs.: ${esc(i.obs)}</div>`:''}</div></div><b>${brl(i.unit*i.qty)}</b></div><div class="cartQty"><button data-pedevia-handler="h078" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">−</button><span class="num">${i.qty}</span><button data-pedevia-handler="h079" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">+</button><button class="cartRemove" data-pedevia-handler="h080" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">×</button></div></div>`}).join('')+`</div>`;
...
function showReceiveChoices(){
 ...
 if(s.modes.pickup)h+=`<button class="choiceTile" data-pedevia-handler="h081"><span class="bigIcon">🛍️</span><span><b>Retirar no estabelecimento</b><small>Aprox. ${o.pickupTime||40} mins</small></span></button>`;
 if(s.modes.dinein)h+=`<button class="choiceTile" data-pedevia-handler="h082"><span class="bigIcon">🍽️</span><span><b>Consumir no local</b><small>${o.dineinTableRequired?'Número da mesa obrigatório':'Informe a mesa se desejar'}</small></span></button>`;
 if(s.modes.delivery)h+=`<button class="choiceTile" data-pedevia-handler="h083"><span class="bigIcon">🚚</span><span><b>Receber por entrega</b><small>${esc(o.deliveryTime||'Entrega')}</small></span></button>`;
 showModal(h);
}
...
function showDeliveryForm(){
 ...
 let bh=mode==='neighborhood'?`<label>Bairro *</label><select id="coNeigh" class="field" data-pedevia-handler="h084">${opts}</select>`:'';
...
function showPaymentChoices(){
 ...
 h+=keys.map(k=>{let meta=m[k]||{},tag=meta.feeType==='fee'&&+meta.fee?`<span class="feeTag">Taxa: ${meta.fee}%</span>`:meta.feeType==='discount'&&+meta.fee?`<span class="feeTag">Desconto: ${meta.fee}%</span>`:'';return `<button class="choiceTile" data-pedevia-handler="h086" data-pedevia-args="${encodeURIComponent(JSON.stringify([k]))}"><span class="bigIcon">${paymentIcon(k)}</span><span><b>${paymentKeyLabel(k)}</b></span>${tag}</button>`}).join('');
 showModal(h);
}
...
function editProduct(id){let p=cfg.products.find(x=>x.id===id); if(!p)return;
  ...
  <div class="prodImageActions"><label class="ghost" style="display:inline-block">📷 Escolher imagem<input id="prodFile" type="file" accept="image/*" hidden data-pedevia-handler="h087" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}"></label><button class="ghost" data-pedevia-handler="h088">🔗 Usar URL</button></div>
  <label>Nome do produto *</label><input id="epn" class="field" value="${esc(p.name)}">
  <label>Descrição</label><textarea id="epd" class="field">${esc(p.desc)}</textarea>
  <label>Descrição detalhada</label><textarea id="epdetail" class="field" placeholder="Opcional">${esc(p.detailedDesc||'')}</textarea>
  ...
  <div id="productGroupList">${productGroupsHTML(p)}</div><button class="ghost full" data-pedevia-handler="h089" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">＋ Adicionar grupo de complementos</button>
  <div class="stickySave"><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">Cancelar</button><button class="btn" data-pedevia-handler="h090" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Concluir</button></div>`);
  $('#eps').value=p.status;
}
...
function showReceiveChoices(){
  ...
  if(a.pickup)h+=`<button class="choiceTile" data-pedevia-handler="h091"><span class="bigIcon">🛍️</span><span><b>Retirar no estabelecimento</b><small>Aprox. ${o.pickupTime||40} mins</small></span></button>`;
  if(a.dinein)h+=`<button class="choiceTile" data-pedevia-handler="h092"><span class="bigIcon">🍽️</span><span><b>Consumir no local</b><small>${o.dineinTableRequired?'Número da mesa obrigatório':'Informe a mesa se desejar'}</small></span></button>`;
  if(a.delivery)h+=`<button class="choiceTile" data-pedevia-handler="h093"><span class="bigIcon">🚚</span><span><b>Receber por entrega</b><small>${esc(o.deliveryTime||'Entrega')}</small></span></button>`;
...
function renderClientServiceBar(){
  ...
  bar.innerHTML=`<div class="serviceModeTitle">Como você quer receber?</div><div class="serviceModeBtns">${s.modes.delivery?`<button class="${m==='delivery'?'on':''}" data-pedevia-handler="h094">🚚 Delivery</button>`:''}${s.modes.pickup?`<button class="${m==='pickup'?'on':''}" data-pedevia-handler="h095">🛍️ Retirada</button>`:''}${s.modes.dinein?`<button class="${m==='dinein'?'on':''}" data-pedevia-handler="h096">🍽️ Consumo local</button>`:''}</div>${serviceRestrictionMessage(m)?`<div class="serviceModeHint">${serviceRestrictionMessage(m)}</div>`:''}`;
...
function renderShop(){
  ...
  $('#categoryTabs').className='quickSections';$('#categoryTabs').innerHTML=secs.map(s=>`<button class="tab" data-pedevia-handler="h097" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">${esc(s.title)}</button>`).join('');
...
function showReceiveChoices(){
  let s=cfg.store,o=s.orderConfig||{},h=`<div class="row"><h2 class="checkoutTitle">Como deseja receber o pedido</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="showCart">✕</button></div>`;
  if(s.modes.pickup)h+=`<button class="choiceTile" data-pedevia-handler="h098"><span class="bigIcon">🛍️</span><span><b>Retirar no estabelecimento</b><small>Aprox. ${o.pickupTime||40} mins</small><small class="modeRuleSmall">Para esta opção, aceitamos somente pote de 1 litro.</small></span></button>`;
  if(s.modes.dinein)h+=`<button class="choiceTile" data-pedevia-handler="h099"><span class="bigIcon">🍽️</span><span><b>Consumir no local</b><small>${o.dineinTableRequired?'Número da mesa obrigatório':'Informe a mesa se desejar'}</small><small class="modeRuleSmall">Para esta opção, aceitamos somente pote de 1 litro.</small></span></button>`;
  if(s.modes.delivery)h+=`<button class="choiceTile" data-pedevia-handler="h100"><span class="bigIcon">🚚</span><span><b>Receber por entrega</b><small>${esc(o.deliveryTime||'Entrega')}</small></span></button>`;
  showModal(h);
}
...
function variantsEditorHTML(p){
  ...
  <label class="switchrow"><span><b>Este produto possui tamanhos</b><small class="hint">Ideal para sabores de sorvete. O sabor aparece no cardápio e o tamanho é escolhido ao abrir.</small></span><input id="pvEnabled" type="checkbox" ${on?'checked':''} data-pedevia-handler="h101" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}"></label>
...
function editProduct(id){let p=cfg.products.find(x=>x.id===id); if(!p)return; normalizeProductModes();
  showModal(`<div class="row"><h2>${p.name==='Novo produto'?'Adicionar produto':'Editar produto'}</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>
  <div class="prodHero" id="prodPrev">${productImagePreview(p)}</div>
  <div class="prodImageActions"><label class="ghost" style="display:inline-block">📷 Escolher imagem<input id="prodFile" type="file" accept="image/*" hidden data-pedevia-handler="h103" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}"></label><button class="ghost" data-pedevia-handler="h104">🔗 Usar URL</button></div>
  <label>Nome do produto *</label><input id="epn" class="field" value="${esc(p.name)}">
  <label>Descrição</label><textarea id="epd" class="field">${esc(p.desc||'')}</textarea>
  <label>Descrição detalhada</label><textarea id="epdetail" class="field" placeholder="Opcional">${esc(p.detailedDesc||'')}</textarea>
  ...
  <div id="productGroupList">${productGroupsHTML(p)}</div><button class="ghost full" data-pedevia-handler="h105" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">＋ Adicionar grupo de complementos</button>
  <div class="stickySave"><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">Cancelar</button><button class="btn" data-pedevia-handler="h106" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Concluir</button></div>`); $('#eps').value=p.status;
...
function openProduct(id){
  ...
  (p.groups||[]).forEach(gid=>{let g=cfg.groups.find(x=>x.id===gid);if(!g)return;let os=(g.options||[]).filter(o=>o.status==='available');if(!os.length)return;let r=effectiveGroupRules(g),maxAttr=Number.isFinite(r.max)?r.max:'inf';h+=`<div class="group choiceGroup" data-gid="${g.id}" data-mode="${r.mode}" data-min="${r.min}" data-max="${maxAttr}"><div class="groupTitle">${esc(g.name)} ${r.required?'<span class="tinyTag">Obrigatório</span>':'<span class="tinyTag">Opcional</span>'}</div><div class="hint">${clientRuleText(g)}</div>`;if(r.mode==='quantity')h+=os.map(o=>quantityOptionRow(g,o)).join('');else{let type=r.mode==='single'?'radio':'checkbox';h+=os.map(o=>`<label class="option optionCard"><span><b>${esc(o.name)}</b>${o.desc?`<small class="hint">${esc(o.desc)}</small>`:''}${o.price?`<span class="optionPrice">+ ${brl(o.price)}</span>`:'<span class="optionPrice">Grátis</span>'}</span><input type="${type}" name="g_${g.id}" value="${o.id}" data-pedevia-event="change" data-pedevia-call="handleChoiceChange" data-pedevia-arg="this"></label>`).join('')}h+='</div>'});
  h+=`<label>Observação</label><textarea id="itemObs" class="field" placeholder="Ex.: sem granola"></textarea><div class="productFooter"><div class="productQty"><button data-pedevia-handler="h107">−</button><b id="qty">1</b><button data-pedevia-handler="h108">+</button></div><button class="btn addPriceBtn" data-base="${base}" data-pedevia-handler="h109" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Adicionar • <span class="liveTotal">${brl(base)}</span></button></div>`;
...
function renderShop(){
  let st=openState();renderClientStoreInfo();renderClientServiceBar();$('#storeStatus').textContent='● '+st.text;$('#closedNotice').innerHTML=st.open?'':'<div class="notice bad">A loja está fechada no momento. Você pode montar o pedido e consultar o cardápio.</div>';
  let secs=(Array.isArray(cfg.sections)?cfg.sections:[]).filter(s=>s.active!==false);if(!secs.length){repairSectionsEmergency();secs=cfg.sections.filter(s=>s.active!==false)}
  $('#categoryTabs').className='quickSections';$('#categoryTabs').innerHTML=secs.map(s=>`<button class="tab" data-pedevia-handler="h110" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">${esc(s.title)}</button>`).join('');
  let mode=window.clientServiceMode||'delivery';
...
function showCart(){let h=`<div class="row"><div><h2 style="margin:0">Seu pedido</h2><div class="hint">Adicione mais produtos ou finalize quando quiser.</div></div><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>`;if(!cart.length){showModal(h+'<div class="emptySection">Seu carrinho está vazio.</div><button class="btn full" data-pedevia-event="click" data-pedevia-call="closeModal">Voltar ao cardápio</button>');return}h+=`<div>`+cart.map((i,n)=>{let p=cfg.products.find(x=>x.id===i.pid);return `<div class="cartItem"><div class="cartItemHead"><div><b>${esc(p?.name||'Produto')}${i.variantName?` · ${esc(i.variantName)}`:''}</b><div class="cartItemMeta">${i.groups.map(g=>`<div>• ${esc(g.name)}: ${g.items.map(itemLabel).join(', ')}</div>`).join('')}${i.obs?`<div>• Obs.: ${esc(i.obs)}</div>`:''}</div></div><b>${brl(i.unit*i.qty)}</b></div><div class="cartQty"><button data-pedevia-handler="h111" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">−</button><span class="num">${i.qty}</span><button data-pedevia-handler="h112" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">+</button><button class="cartRemove" data-pedevia-handler="h113" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">×</button></div></div>`}).join('')+`</div><div class="cartTotals"><div class="row"><span>Valor dos produtos</span><b>${brl(sum())}</b></div></div><div class="cartActions"><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">Adicionar mais produtos</button><button class="btn" data-pedevia-event="click" data-pedevia-call="showReceiveChoices">Finalizar pedido ›</button></div>`;showModal(h)}
...
function renderShop(){
  ...
  $('#categoryTabs').innerHTML=secs.map(s=>`<button class="tab" data-pedevia-handler="h114" data-pedevia-args="${encodeURIComponent(JSON.stringify([s.id]))}">${esc(s.title)}</button>`).join('');
...
function openProduct(id){
  ...
  (p.groups||[]).forEach(gid=>{let g=cfg.groups.find(x=>x.id===gid);if(!g)return;let os=(g.options||[]).filter(o=>o.status==='available');if(!os.length)return;let r=effectiveGroupRules(g),maxAttr=Number.isFinite(r.max)?r.max:'inf';h+=`<div class="group choiceGroup" data-gid="${g.id}" data-mode="${r.mode}" data-min="${r.min}" data-max="${maxAttr}"><div class="groupTitle">${esc(g.name)} ${r.required?'<span class="tinyTag">Obrigatório</span>':'<span class="tinyTag">Opcional</span>'}</div><div class="hint">${clientRuleText(g)}</div>`;if(r.mode==='quantity')h+=os.map(o=>quantityOptionRow(g,o)).join('');else{let type=r.mode==='single'?'radio':'checkbox';h+=os.map(o=>`<label class="option optionCard"><span><b>${esc(o.name)}</b>${o.desc?`<small class="hint">${esc(o.desc)}</small>`:''}${o.price?`<span class="optionPrice">+ ${brl(o.price)}</span>`:'<span class="optionPrice">Grátis</span>'}</span><input type="${type}" name="g_${g.id}" value="${o.id}" data-pedevia-event="change" data-pedevia-call="handleChoiceChange" data-pedevia-arg="this"></label>`).join('')}h+='</div>'});
  h+=`<label>Observação</label><textarea id="itemObs" class="field" placeholder="Ex.: sem granola"></textarea><div class="productFooter"><div class="productQty"><button data-pedevia-handler="h115">−</button><b id="qty">1</b><button data-pedevia-handler="h116">+</button></div><button class="btn addPriceBtn" data-base="${base}" data-pedevia-handler="h117" data-pedevia-args="${encodeURIComponent(JSON.stringify([p.id]))}">Adicionar • <span class="liveTotal">${brl(base)}</span></button></div>`;
  showModal(h);setTimeout(()=>{refreshAllGroupLimits();if(firstVar){let i=document.querySelector(`input[name="productVariant"][value="${CSS.escape(firstVar.id)}"]`);if(i){i.checked=true;handleVariantChange()}}},0);
}
...
function showReceiveChoices(){
  ...
  if(a.pickup)h+=`<button class="choiceTile" data-pedevia-handler="h118"><span class="bigIcon">🛍️</span><span><b>Retirar no estabelecimento</b><small>Aprox. ${o.pickupTime||40} mins</small></span></button>`;
  if(a.dinein)h+=`<button class="choiceTile" data-pedevia-handler="h119"><span class="bigIcon">🍽️</span><span><b>Consumir no local</b><small>${o.dineinTableRequired?'Número da mesa obrigatório':'Informe a mesa se desejar'}</small></span></button>`;
  if(a.delivery)h+=`<button class="choiceTile" data-pedevia-handler="h120"><span class="bigIcon">🚚</span><span><b>Receber por entrega</b><small>${esc(o.deliveryTime||'Entrega')}</small></span></button>`;
...
function adminOffers(){
 normalizeOffers();
 let list=cfg.offers.map(o=>`<div class="offerAdminCard"><div class="row"><div><span class="offerType">${o.type==='percentage'?'DESCONTO':'FIDELIDADE'}</span><h3 style="margin:5px 0 3px">${esc(o.title)}</h3><div class="hint">${o.type==='percentage'?`${+o.percent||0}% · ${esc(offerScopeText(o))}${+o.minValue?' · mínimo '+brl(+o.minValue):''}`:`${o.requiredOrders||10} pedidos → ${o.rewardPercent||10}% de benefício`}</div></div><span class="badge ${o.active?'available':'hidden'}">${o.active?'Ativa':'Inativa'}</span></div><div class="miniBtns" style="margin-top:10px"><button class="ghost" data-pedevia-handler="h121" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Editar</button><button class="ghost" data-pedevia-handler="h122" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">${o.active?'Desativar':'Ativar'}</button><button class="dangerBtn" data-pedevia-handler="h123" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Excluir</button></div></div>`).join('')||'<div class="emptySection">Nenhuma oferta criada ainda.</div>';
 $('#adminContent').innerHTML=`<div class="panel"><div class="menuBuilderHead"><div><h3 style="margin:0">Ofertas</h3><div class="hint">Crie promoções que aparecem no cardápio e podem alterar o total do pedido.</div></div></div><div class="two"><button class="btn" data-pedevia-handler="h124">＋ Desconto</button><button class="ghost" data-pedevia-handler="h125">★ Fidelidade</button></div>${list}<div class="notice">Descontos percentuais ativos são aplicados automaticamente no carrinho. Se houver mais de um válido, o sistema usa o que dá o maior desconto.</div></div>`;
}
...
function editOffer(id){let o=cfg.offers.find(x=>x.id===id);if(!o)return;
 if(o.type==='loyalty'){
   showModal(`<div class="row"><h2>Programa de fidelidade</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div><label>Nome</label><input id="ofTitle" class="field" value="${esc(o.title)}"><label>Descrição para o cliente</label><textarea id="ofDesc" class="field">${esc(o.description||'')}</textarea><div class="two"><div><label>Quantidade de pedidos</label><input id="ofOrders" type="number" min="1" class="field" value="${o.requiredOrders||10}"></div><div><label>Benefício (%)</label><input id="ofReward" type="number" min="0" max="100" class="field" value="${o.rewardPercent||10}"></div></div><label class="switchrow"><span>Programa ativo</span><input id="ofActive" type="checkbox" ${o.active?'checked':''}></label><div class="notice">O programa já aparece no cardápio e o progresso dos clientes é controlado automaticamente pelos pedidos registrados.</div><button class="btn full" data-pedevia-handler="h126" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}">Salvar fidelidade</button>`);return;
 }
 showModal(`<div class="row"><h2>Editar desconto</h2><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div><label>Nome da oferta</label><input id="ofTitle" class="field" value="${esc(o.title)}"><label>Descrição para o cliente</label><textarea id="ofDesc" class="field">${esc(o.description||'')}</textarea><div class="two"><div><label>Desconto (%)</label><input id="ofPercent" type="number" min="0" max="100" class="field" value="${+o.percent||0}"></div><div><label>Pedido mínimo (R$)</label><input id="ofMin" type="number" min="0" step=".01" class="field" value="${+o.minValue||0}"></div></div><label>Aplicar em</label><select id="ofScope" class="field" data-pedevia-handler="h127" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}"><option value="all" ${o.scope==='all'?'selected':''}>Todo o cardápio</option><option value="section" ${o.scope==='section'?'selected':''}>Uma seção</option><option value="product" ${o.scope==='product'?'selected':''}>Um produto</option></select><div id="offerTargetWrap"></div><label class="switchrow"><span>Oferta ativa</span><input id="ofActive" type="checkbox" ${o.active?'checked':''}></label><button class="btn full" data-pedevia-handler="h128" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}">Salvar oferta</button>`);setTimeout(()=>refreshOfferTarget(id),0)
}
...
function showCart(){
 ...
 let h=`<div class="row"><div><h2 class="checkoutTitle">Seu pedido</h2><div class="checkoutSub">Revise os itens ou continue comprando</div></div><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>`;
 if(!cart.length){showModal(h+`<div class="notice">Seu carrinho está vazio.</div><button class="btn full" data-pedevia-event="click" data-pedevia-call="continueShopping">Voltar ao cardápio</button>`);return}
 h+=`<div>`+cart.map((i,n)=>{let p=cfg.products.find(x=>x.id===i.pid);return `<div class="cartItem"><div class="cartItemHead"><div><b>${esc(p?.name||'Produto')}${i.variantName?' · '+esc(i.variantName):''}</b><div class="cartItemMeta">${i.groups.map(g=>`<div>• ${esc(g.name)}: ${g.items.map(itemLabel).join(', ')}</div>`).join('')}${i.obs?`<div>• Obs.: ${esc(i.obs)}</div>`:''}</div></div><b>${brl(i.unit*i.qty)}</b></div><div class="cartQty"><button data-pedevia-handler="h129" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">−</button><span class="num">${i.qty}</span><button data-pedevia-handler="h130" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">+</button><button class="cartRemove" data-pedevia-handler="h131" data-pedevia-args="${encodeURIComponent(JSON.stringify([n]))}">×</button></div></div>`}).join('')+`</div>`;
...
function showCropperModal(title){
  ...
  showModal(`
    ...
    <label>Zoom</label>
    <input id="cropZoom" type="range" min="1" max="3" step="0.01" value="1" class="field" data-pedevia-handler="h132">
    <div class="two">
