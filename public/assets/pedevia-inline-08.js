...
function showTenantNotFoundV121(slug){
  ...
  if(shop)shop.innerHTML=`<div class="panel" style="max-width:620px;margin:50px auto;text-align:center"><div style="font-size:52px">🏪</div><h2>Loja indisponível</h2><p class="hint">O endereço <b>/${esc(slug)}</b> não está publicado ou não existe na Pedevia.</p><button class="btn" data-pedevia-handler="h146">Ir para a Pedevia</button></div>`;
  ...
  el.innerHTML=rows.map(r=>{
    ...
    return `<div class="adminItem" style="align-items:flex-start">
      ...
      <div class="miniBtns" style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">
        <button class="ghost" data-pedevia-handler="h147" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Abrir site</button>
        <button class="ghost" data-pedevia-handler="h148" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Copiar link</button>
        <button class="ghost" data-pedevia-handler="h149" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Gerenciar</button>
      </div>
    </div>`;
  }).join('');
};
...
function renderClientLogoPreviewV1211(){
  ...
  box.innerHTML=window.clientLogoDraftV1211
    ? `<div style="display:flex;align-items:center;gap:12px"><img src="${window.clientLogoDraftV1211}" style="width:88px;height:88px;object-fit:contain;border-radius:18px;border:1px solid #ddd;background:#fff"><button class="ghost" type="button" data-pedevia-handler="h150">Remover logo</button></div>`
...
editClientSiteV120=function(id){
  ...
  <div class="panel" style="margin:14px 0;padding:14px"><b>📧 Acesso do administrador</b><div class="hint" style="margin:4px 0 10px">Administrador atual: <b>${esc(r.admin_email||'E-mail não informado')}</b>. Use o botão abaixo para enviar o primeiro convite ou reenviar o acesso.</div><button id="resendInviteV122" type="button" class="btn full" data-pedevia-handler="h151" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Enviar / reenviar convite de acesso</button></div>
  <div class="panel" style="margin:14px 0;padding:14px"><b>Logo do estabelecimento</b><div class="hint" style="margin:4px 0 10px">A mesma imagem será usada no logo principal e no logozinho do topo. Se não enviar uma imagem, o site mostra apenas “LOGO”.</div><div id="clientLogoPreviewV1211"></div><label class="btn" style="display:inline-block;margin-top:10px;cursor:pointer">Enviar logo<input type="file" accept="image/*" style="display:none" data-pedevia-handler="h152"></label></div>
  ...
  <div class="panel" style="margin:14px 0;padding:14px"><b>▦ QR Code enviado pelo cliente</b><div class="hint" style="margin:4px 0 10px">Imagem do QR Code salva junto ao cadastro desta empresa.</div><div id="clientQrPreviewV1201"></div><label class="btn" style="display:inline-block;margin-top:10px;cursor:pointer">Enviar QR Code<input type="file" accept="image/*" style="display:none" data-pedevia-handler="h153"></label></div>
  <button id="cseSaveBtnV120" class="btn full" data-pedevia-handler="h154" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Salvar</button>
  <button class="dangerBtn full" style="margin-top:8px" data-pedevia-handler="h155" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Excluir esta loja</button>`);
...
newClientSiteV120=function(){
  ...
  <label>Nome do estabelecimento</label><input id="csNameV120" class="field" placeholder="Ex.: Pizzaria Central" data-pedevia-handler="h156">
...
showPedeviaWelcomeV123=function(){
  const name=tenantNameV1242();
  showModal(`<div style="text-align:center;padding:6px 2px 2px"><div style="width:74px;height:74px;border-radius:24px;margin:0 auto 14px;display:grid;place-items:center;background:var(--primary,#712489);color:#fff;font-size:34px;font-weight:900">P</div><div class="hint">PEDEVIA</div><h2 style="font-size:27px;margin:6px 0 8px">Bem-vindo à sua nova loja! 👋</h2><p style="line-height:1.55;margin:0 auto 16px;max-width:420px">O acesso de <b>${esc(name)}</b> está pronto. Agora você pode aprender usando as telas reais da sua loja.</p><div style="background:rgba(0,0,0,.035);padding:13px;border-radius:16px;text-align:left;margin-bottom:16px"><b>Tutorial prático:</b><div class="hint" style="margin-top:6px;line-height:1.5">O guia abre cada configuração, explica o que fazer e acompanha você enquanto realiza alterações reais. Ao salvar uma etapa, seguimos automaticamente para a próxima.</div></div><button class="btn full" data-pedevia-handler="h159">Começar tutorial prático →</button><button class="ghost full" style="margin-top:8px" data-pedevia-event="click" data-pedevia-call="skipWelcomeV123">Pular tutorial e ir ao painel</button></div>`);
};
...
async function loadOrdersPanelV125(){
  ...
  try{
    ...
    const body=rows.length?rows.map(o=>`<div class="panel" style="padding:13px"><div class="row"><div><b style="font-size:17px">${orderNumberV125(o)}</b> <span class="badge ${orderStatusClassV125(o.status)}">${orderStatusLabelV125(o.status)}</span><div class="hint">${orderDateV125(o.created_at)} · ${esc(o.customer_name||'Cliente')} · ${orderModeLabelV125(o.order_mode)}</div></div><b>${brl(+o.total||0)}</b></div><div class="miniBtns" style="margin-top:10px"><button class="ghost" data-pedevia-handler="h162" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Ver pedido</button><button class="ghost" data-pedevia-handler="h163" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Copiar</button></div></div>`).join(''):'<div class="panel"><h3>Nenhum pedido ainda</h3><p class="hint">Assim que um cliente finalizar um pedido, ele aparecerá aqui automaticamente.</p></div>';
...
adminOrders=function(){
  ...
  document.getElementById('adminContent').innerHTML=`<div class="orderHero"><div class="row"><div><h2 style="margin:0">Pedidos</h2><div class="hint">Todos os pedidos feitos pelo cardápio aparecem aqui.</div></div><button class="ghost" data-pedevia-handler="h164">⚙ Configurar</button></div><div class="storeState ${paused?'paused':''}"><div><span class="stateDot"></span><b>${paused?'Recebimento pausado':st.open?'Estabelecimento aberto':'Estabelecimento fechado'}</b><div style="font-size:12px;opacity:.8">${paused?'Pedidos online temporariamente pausados':st.text}</div></div><button data-pedevia-event="click" data-pedevia-call="toggleOrdersPaused">${paused?'Retomar':'Pausar'} ▾</button></div></div><div id="ordersListV125"></div>`;
  loadOrdersPanelV125();
};
...
viewOrderV125=function(id){
  const o=(window.pedeviaOrdersV125||[]).find(x=>String(x.id)===String(id));if(!o)return;
  const items=(o.items||[]).map(i=>`<div class="summary"><div class="row"><b>${i.quantity||1}x ${esc(i.name||'Produto')}${i.variant?' · '+esc(i.variant):''}</b><b>${brl((+i.unit_price||0)*(+i.quantity||1))}</b></div>${(i.groups||[]).map(g=>`<small><b>${esc(g.name||'Complementos')}:</b> ${(g.items||[]).map(x=>esc(x.name||x)).join(', ')}</small>`).join('')}${i.note?`<small>Obs.: ${esc(i.note)}</small>`:''}</div>`).join('');
  showModal(`<div class="row"><div><h2 style="margin:0">Pedido ${orderNumberV125(o)}</h2><div class="hint">${orderDateV125(o.created_at)}</div></div><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>${items}<div class="summary"><div><b>${esc(o.customer_name||'-')}</b> · ${esc(o.customer_phone||'-')}</div><small>${orderModeLabelV125(o.order_mode)}${o.neighborhood?' · '+esc(o.neighborhood):''}${o.address?' · '+esc(o.address):''}${o.table_number?' · Mesa '+esc(o.table_number):''}</small><small>Pagamento: ${esc(paymentKeyLabel(o.payment_method))}</small>${o.note?`<small>Obs.: ${esc(o.note)}</small>`:''}<div class="row" style="margin-top:8px"><strong>Total</strong><strong>${brl(+o.total||0)}</strong></div></div><label>Status</label><select id="orderStatusV125" class="field"><option value="new">Novo</option><option value="accepted">Aceito</option><option value="preparing">Preparando</option><option value="ready">Pronto</option><option value="completed">Concluído</option><option value="cancelled">Cancelado</option></select><button class="btn full" data-pedevia-handler="h165" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Salvar status</button><button class="ghost full" style="margin-top:8px" data-pedevia-handler="h166" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Copiar pedido</button><button class="dangerBtn full" style="width:100%;margin-top:8px" data-pedevia-handler="h167" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Excluir pedido definitivamente</button>`);
  setTimeout(()=>{const s=document.getElementById('orderStatusV125');if(s)s.value=o.status||'new'},0);
};

function renderOrderCardV126(o){
  return `<div class="panel" style="padding:13px"><div class="row"><div><b style="font-size:17px">${orderNumberV125(o)}</b> <span class="badge ${orderStatusClassV125(o.status)}">${orderStatusLabelV125(o.status)}</span><div class="hint">${orderDateV125(o.created_at)} · ${esc(o.customer_name||'Cliente')} · ${orderModeLabelV125(o.order_mode)}</div></div><b>${brl(+o.total||0)}</b></div><div class="v126OrderActions"><button class="ghost" data-pedevia-handler="h168" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Ver pedido</button><button class="ghost" data-pedevia-handler="h169" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Copiar</button>${archivedOrderV126(o)?`<button class="ghost v126Danger" data-pedevia-handler="h170" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Excluir</button>`:''}</div></div>`;
}
...
async function loadOrdersPanelV126(){
  ...
  try{
    ...
    const tabs=`<div class="v126Filters"><button class="${ordersTabV126==='active'?'on':''}" data-pedevia-handler="h171">Ativos (${active.length})</button><button class="${ordersTabV126==='history'?'on':''}" data-pedevia-handler="h172">Histórico (${history.length})</button></div>`;
    const actions=ordersTabV126==='active'?`<div class="v126Toolbar"><button class="ghost" data-pedevia-event="click" data-pedevia-call="finishTodayOrdersV126">✓ Encerrar pedidos de hoje</button><button class="ghost" data-pedevia-handler="h173">📊 Estatísticas</button></div>`:`<div class="v126Toolbar"><button class="ghost" data-pedevia-handler="h174">📊 Estatísticas</button></div>`;
...
async function loadStatisticsV126(){
  ...
  try{
    ...
    const ranges=[['7','7 dias'],['30','30 dias'],['90','90 dias'],['all','Todo período']].map(([v,n])=>`<button class="${statsRangeV126===v?'on':''}" data-pedevia-handler="h175" data-pedevia-args="${encodeURIComponent(JSON.stringify([v]))}">${n}</button>`).join('');
    const topCustomers=customers.filter(c=>c.completed>0).slice(0,10).map((c,i)=>`<div class="v126Rank"><div class="v126RankNum">${i+1}</div><div><b>${esc(c.name)}</b><small>${esc(c.phone||'Sem telefone')} · ${c.completed} concluído(s)</small></div><div class="v126Right"><b>${brl(c.spent)}</b><button class="ghost" style="padding:5px 8px;margin-top:4px" data-pedevia-handler="h176" data-pedevia-args="${encodeURIComponent(JSON.stringify([c.key]))}">Histórico</button></div></div>`).join('')||'<p class="hint">Ainda não há clientes com pedidos concluídos neste período.</p>';
    const allCustomers=customers.slice(0,30).map(c=>`<div class="adminItem"><div><b>${esc(c.name)}</b><small>${esc(c.phone||'Sem telefone')} · último pedido ${orderDateV125(c.last)}</small></div><button class="ghost" data-pedevia-handler="h177" data-pedevia-args="${encodeURIComponent(JSON.stringify([c.key]))}">Ver histórico</button></div>`).join('')||'<p class="hint">Nenhum cliente registrado.</p>';
...
function adminStatisticsV126(){
  document.getElementById('adminContent').innerHTML=`<button class="backBtn" data-pedevia-handler="h178">‹ Voltar para Pedidos</button><div class="panel"><div class="row"><div><div class="settingsTitle" style="margin-bottom:2px">Estatísticas de vendas</div><div class="hint">Faturamento considera somente pedidos marcados como Concluídos.</div></div></div></div><div id="statsHostV126"></div>`;loadStatisticsV126();
}
...
adminOrders=function(){
  ...
  const st=openState(),paused=cfg.store.paused;
  document.getElementById('adminContent').innerHTML=`<div class="orderHero"><div class="row"><div><h2 style="margin:0">Pedidos</h2><div class="hint">Acompanhe os pedidos ativos e consulte o histórico.</div></div><button class="ghost" data-pedevia-handler="h179">⚙ Configurar</button></div><div class="storeState ${paused?'paused':''}"><div><span class="stateDot"></span><b>${paused?'Recebimento pausado':st.open?'Estabelecimento aberto':'Estabelecimento fechado'}</b><div style="font-size:12px;opacity:.8">${paused?'Pedidos online temporariamente pausados':st.text}</div></div><button data-pedevia-event="click" data-pedevia-call="toggleOrdersPaused">${paused?'Retomar':'Pausar'} ▾</button></div></div><div id="ordersListV125"></div>`;loadOrdersPanelV126();
};
...
function specialClosuresHtmlV127(){const arr=cfg.store.specialClosures||[];return arr.length?arr.slice().sort((a,b)=>a.date.localeCompare(b.date)).map((x,i)=>`<div class="v127Exception"><div><b>${esc(new Date(x.date+'T12:00:00').toLocaleDateString('pt-BR'))}</b><small>${esc(x.message||'Fechado excepcionalmente')}</small></div><button class="ghost" data-pedevia-handler="h180" data-pedevia-args="${encodeURIComponent(JSON.stringify([i]))}">Remover</button></div>`).join(''):'<p class="hint">Nenhum fechamento excepcional cadastrado.</p>'}
...
updateOrderStatusV125=updateOrderStatusV126;
viewOrderV125=function(id){const o=(window.pedeviaOrdersV125||[]).find(x=>String(x.id)===String(id));if(!o)return;const items=(o.items||[]).map(i=>`<div class="summary"><div class="row"><b>${i.quantity||1}x ${esc(i.name||'Produto')}${i.variant?' · '+esc(i.variant):''}</b><b>${brl((+i.unit_price||0)*(+i.quantity||1))}</b></div>${(i.groups||[]).map(g=>`<small><b>${esc(g.name||'Complementos')}:</b> ${(g.items||[]).map(x=>esc(x.name||x)).join(', ')}</small>`).join('')}${i.note?`<small>Obs.: ${esc(i.note)}</small>`:''}</div>`).join('');showModal(`<div class="row"><div><h2 style="margin:0">Pedido ${orderNumberV125(o)}</h2><div class="hint">${orderDateV125(o.created_at)}</div></div><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>${items}<div class="summary"><div><b>${esc(o.customer_name||'-')}</b> · ${esc(o.customer_phone||'-')}</div><small>${orderModeLabelV125(o.order_mode)}${o.neighborhood?' · '+esc(o.neighborhood):''}${o.address?' · '+esc(o.address):''}${o.table_number?' · Mesa '+esc(o.table_number):''}</small><small>Pagamento: ${esc(paymentKeyLabel(o.payment_method))}</small>${o.note?`<small>Obs.: ${esc(o.note)}</small>`:''}<div class="row" style="margin-top:8px"><strong>Total</strong><strong>${brl(+o.total||0)}</strong></div></div>${orderTimelineV127(o)}<label>Status</label><select id="orderStatusV125" class="field"><option value="new">Novo</option><option value="accepted">Aceito</option><option value="preparing">Preparando</option><option value="ready">Pronto</option><option value="completed">Concluído</option><option value="cancelled">Cancelado</option></select><button class="btn full" data-pedevia-handler="h181" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Salvar status</button><div class="v126OrderActions"><button class="ghost" data-pedevia-handler="h182" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">WhatsApp do cliente</button><button class="ghost" data-pedevia-handler="h183" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Copiar</button><button class="ghost" data-pedevia-handler="h184" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Imprimir</button></div><button class="dangerBtn full" style="width:100%;margin-top:8px" data-pedevia-handler="h185" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Excluir pedido definitivamente</button>`);setTimeout(()=>{const s=document.getElementById('orderStatusV125');if(s)s.value=o.status||'new'},0)};
renderOrderCardV126=function(o){return `<div class="panel" style="padding:13px"><div class="row"><div><b style="font-size:17px">${orderNumberV125(o)}</b> <span class="badge ${orderStatusClassV125(o.status)}">${orderStatusLabelV125(o.status)}</span><div class="hint">${orderDateV125(o.created_at)} · ${esc(o.customer_name||'Cliente')} · ${orderModeLabelV125(o.order_mode)}</div></div><b>${brl(+o.total||0)}</b></div><div class="v126OrderActions"><button class="ghost" data-pedevia-handler="h186" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Ver pedido</button><button class="ghost" data-pedevia-handler="h187" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">WhatsApp</button><button class="ghost" data-pedevia-handler="h188" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Imprimir</button><button class="ghost" data-pedevia-handler="h189" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Copiar</button>${archivedOrderV126(o)?`<button class="ghost v126Danger" data-pedevia-handler="h190" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Excluir</button>`:''}</div></div>`};
...
window.PedeviaV130 = window.PedeviaV130 || {
  ...
  orderCard(o){
    ...
    return `<article class="v130OrderCard ${ageClass}">
      ...
      ${address?`<button class="v130Address" data-pedevia-handler="h192" data-pedevia-args="${encodeURIComponent(JSON.stringify([encodeURIComponent(address)]))}">⌖ ${esc(address)}</button>`:''}
      <div class="v130Mini">${(o.items||[]).slice(0,3).map(i=>`${i.quantity||1}x ${esc(i.name||'Produto')}`).join(' · ')}${(o.items||[]).length>3?' +'+((o.items||[]).length-3):''}</div>
      ${next?`<button class="btn full v130Next" data-pedevia-handler="h193" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id,next]))}">${this.nextLabel(o.status)}</button>`:''}
      <div class="v130CardActions"><button data-pedevia-handler="h194" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Ver</button><button data-pedevia-handler="h195" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">WhatsApp</button><button data-pedevia-handler="h196" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Imprimir</button></div>
    </article>`;
  },
  ...
  renderOrders(){
    const st=openState(),paused=cfg.store.paused;
    document.getElementById('adminContent').innerHTML=`<div class="v130OrdersHead"><div><h2>Pedidos</h2><div class="hint">Operação em tempo real · ${esc(st.text||'')}</div></div><div class="v130HeadBtns"><button class="ghost" data-pedevia-handler="h197">Estatísticas</button><button class="ghost" data-pedevia-handler="h198">Histórico</button><button class="ghost" data-pedevia-handler="h199">⚙</button></div></div><div class="storeState ${paused?'paused':''}"><div><span class="stateDot"></span><b>${paused?'Recebimento pausado':st.open?'Estabelecimento aberto':'Estabelecimento fechado'}</b><div style="font-size:12px;opacity:.8">${paused?'Pedidos online temporariamente pausados':st.text}</div></div><button data-pedevia-event="click" data-pedevia-call="toggleOrdersPaused">${paused?'Retomar':'Pausar'} ▾</button></div><div id="ordersListV130"></div>`;
    this.loadBoard();
  },
  renderHistory(){
    ...
    if(!host)return;
    host.innerHTML=`<div class="v130OrdersHead"><div><h2>Histórico de pedidos</h2><div class="hint">Pedidos concluídos e cancelados ficam guardados aqui.</div></div><div class="v130HeadBtns"><button class="ghost" data-pedevia-handler="h200">← Pedidos ativos</button><button class="ghost" data-pedevia-handler="h201">Estatísticas</button></div></div><div id="ordersListV125"></div>`;
  ...
  enhanceShop(){
    ...
    if(!document.getElementById('v130ShopTools')){const d=document.createElement('div');d.id='v130ShopTools';d.className='v130ShopTools';d.innerHTML=`<div class="v130Search"><span>⌕</span><input id="v130SearchInput" placeholder="Buscar produto" data-pedevia-handler="h202"></div>`;tabs.parentNode.insertBefore(d,tabs)}
  ...
  enhanceAdminProducts(){
    document.querySelectorAll('#adminContent .adminProdCard').forEach(card=>{
      ...
      const tools=document.createElement('div');tools.className='v130ProdTools';tools.innerHTML=`<button class="ghost" data-pedevia-handler="h203" data-pedevia-args="${encodeURIComponent(JSON.stringify([id]))}">Disponível ↔</button>`;card.appendChild(tools)
    })
  },
  ...
  specialHoursHtml(){return (cfg.store.specialHours||[]).map(x=>`<div class="v127Exception"><div><b>${esc(new Date(x.date+'T12:00:00').toLocaleDateString('pt-BR'))}</b><small>${esc((x.ranges||[]).map(r=>r.join('–')).join(', '))}</small></div><button class="ghost" data-pedevia-handler="h204" data-pedevia-args="${encodeURIComponent(JSON.stringify([x.date]))}">Remover</button></div>`).join('')||'<p class="hint">Nenhum horário especial.</p>'},
  ...
  init(){this.ensureDefaults();setTimeout(()=>{try{this.enhanceShop()}catch(e){}},400);if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js?v=13248b').catch(()=>{})}
};
...
PedeviaV130.enhanceAdminProducts=function(){
  ...
  if(first&&!document.getElementById('v130BulkToolbar')){const bar=document.createElement('div');bar.id='v130BulkToolbar';bar.className='v130BulkToolbar';bar.innerHTML=`<b>Ações em lote</b><button class="ghost" data-pedevia-handler="h208">Disponibilizar</button><button class="ghost" data-pedevia-handler="h209">Indisponibilizar</button><button class="ghost" data-pedevia-handler="h210">Ocultar</button>`;first.parentNode.insertBefore(bar,first)}
  ...
  renderClientSitesListV120=function(rows){
    ...
    el.innerHTML=`<div class="v126StatsGrid" id="masterSummaryV131">
      ...
      const quick=s==='suspended'||s==='cancelled'||s==='draft'
        ?`<button class="ghost" data-pedevia-handler="h211" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Reativar</button>`
        :`<button class="ghost" data-pedevia-handler="h212" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Suspender</button>`;
      return `<div class="adminItem" style="align-items:flex-start">
        ...
        <div class="miniBtns" style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">
          <button class="ghost" data-pedevia-handler="h213" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Abrir site</button>
          <button class="ghost" data-pedevia-handler="h214" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Copiar link</button>
          ${quick}
          <button class="ghost" data-pedevia-handler="h215" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Gerenciar</button>
        </div>
      </div>`;
    }).join('');
  };
    ...
    showModal(`<div class="row"><div><h2 style="margin:0">${esc(r.name)}</h2><div class="hint">Gestão do estabelecimento na Pedevia</div></div><button class="ghost" data-pedevia-event="click" data-pedevia-call="closeModal">✕</button></div>
      <div class="panel" style="padding:13px;margin:12px 0">
        ...
        <div class="miniBtns" style="margin-top:10px;display:flex;gap:7px;flex-wrap:wrap">
          ${status!=='active'?`<button class="btn" data-pedevia-handler="h216" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">🟢 Reativar estabelecimento</button>`:''}
          ${status!=='suspended'?`<button class="ghost" data-pedevia-handler="h217" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">🟡 Suspender</button>`:''}
          ${status!=='cancelled'?`<button class="dangerBtn" data-pedevia-handler="h218" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">🔴 Cancelar serviço</button>`:''}
        </div>
      ...
      <label>WhatsApp</label><input id="cseWaV120" class="field" value="${escapeAttrV120(r.whatsapp||'')}">
      <label>Administrador</label><input id="cseEmailV120" type="email" class="field" value="${escapeAttrV120(r.admin_email||'')}">
      <div class="panel" style="margin:14px 0;padding:14px"><b>Logo do estabelecimento</b><div id="clientLogoPreviewV1211"></div><label class="btn" style="display:inline-block;margin-top:10px;cursor:pointer">Enviar logo<input type="file" accept="image/*" style="display:none" data-pedevia-handler="h219"></label></div>
      ...
      <button id="cseSaveBtnV120" class="btn full" data-pedevia-handler="h220" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Salvar alterações</button>
      <button class="ghost full" style="margin-top:8px" data-pedevia-handler="h221" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Reenviar convite de acesso</button>
      <button class="dangerBtn full" style="margin-top:8px" data-pedevia-handler="h222" data-pedevia-args="${encodeURIComponent(JSON.stringify([r.id]))}">Excluir definitivamente esta loja</button>`);
