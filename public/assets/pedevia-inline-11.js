...
(function(){
  ...
  adminOffers=function(){
    normalizeOffers();
    let list=cfg.offers.map(o=>`<div class="offerAdminCard"><div class="row"><div><span class="offerType">${o.type==='percentage'?'DESCONTO':'FIDELIDADE'}</span><h3 style="margin:5px 0 3px">${esc(o.title)}</h3><div class="hint">${o.type==='percentage'?`${+o.percent||0}% · ${esc(offerScopeText(o))}${+o.minValue?' · mínimo '+brl(+o.minValue):''}`:`${o.requiredOrders||10} pedidos → ${o.rewardPercent||10}% de benefício`}</div></div><span class="badge ${o.active?'available':'hidden'}">${o.active?'Ativa':'Inativa'}</span></div><div class="miniBtns" style="margin-top:10px"><button class="ghost" data-pedevia-handler="h223" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Editar</button>${o.type==='loyalty'?`<button class="ghost" data-pedevia-event="click" data-pedevia-call="openLoyaltyCustomersAdminV1314">Clientes do clube</button>`:''}<button class="ghost" data-pedevia-handler="h224" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">${o.active?'Desativar':'Ativar'}</button><button class="dangerBtn" data-pedevia-handler="h225" data-pedevia-args="${encodeURIComponent(JSON.stringify([o.id]))}">Excluir</button></div></div>`).join('')||'<div class="emptySection">Nenhuma oferta criada ainda.</div>';

    $('#adminContent').innerHTML=`<div class="panel"><div class="menuBuilderHead"><div><h3 style="margin:0">Ofertas</h3><div class="hint">Crie promoções que aparecem no cardápio e podem alterar o total do pedido.</div></div></div><div class="two"><button class="btn" data-pedevia-handler="h226">＋ Desconto</button><button class="ghost" data-pedevia-handler="h227">★ Fidelidade</button></div>${list}<div class="notice">Descontos percentuais ativos são aplicados automaticamente no carrinho. Se houver mais de um válido, o sistema usa o que dá o maior desconto.</div></div>`;
  };

  setTimeout(()=>{
    document.querySelectorAll('.adminHead .hint').forEach(el=>{
      if(/Versão\s+1\.[0-9.]+/i.test(el.textContent||'')){
        el.textContent=(el.textContent||'').replace(/Versão\s+1\.[0-9.]+/i,'Versão 1.32.48');
      }
    });
  },1200);
